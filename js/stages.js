...
export const STAGES = [
  ...
  {
    id: 'delhi', num: '1-2', name: 'GUTTER GHAT', sub: 'ACT II - THE CHAAT KING', width: 5600,
    wallKey: 'bg_gutter_wall', floorKey: 'bg_gutter_floor', floorW: 960,
    music: 'stage1', bossMusic: 'boss', boss: 'mirchi',
    lamps: [260, 700, 1140, 1580, 2020, 2460, 2900, 3340], lampCol: '255,190,110', lampA: 0.15,
    rim: null,
    moteCount: 34, moteStyle: 'dust',
    // authored warm light where the plates actually show a tandoor or a lamp
    glows: [
      { x: 300, y: 150, r: 34 }, { x: 980, y: 148, r: 30 },
      { x: 1640, y: 150, r: 34 }, { x: 2280, y: 148, r: 30 },
    ],
    // Breakables, kept clear of the stall holders so nothing overlaps them.
    props: [
      { kind: 'crate', x: 300, y: 200 }, { kind: 'matka', x: 348, y: 232 },
      { kind: 'matka', x: 368, y: 236 }, { kind: 'sign', x: 620, y: 192 },
      { kind: 'table', x: 660, y: 226 }, { kind: 'tyres', x: 950, y: 234 },
      { kind: 'crate', x: 1000, y: 198 },
      { kind: 'matka', x: 1320, y: 196 }, { kind: 'table', x: 1560, y: 230 },
      { kind: 'crate', x: 1620, y: 200 }, { kind: 'tyres', x: 1900, y: 236 },
      { kind: 'matka', x: 1960, y: 194 },
      { kind: 'sign', x: 2260, y: 190 }, { kind: 'crate', x: 2470, y: 200 },
      { kind: 'matka', x: 2610, y: 232 },
      { kind: 'table', x: 2860, y: 228 }, { kind: 'crate', x: 3010, y: 200 },
      { kind: 'cart', x: 3250, y: 226 },
    ],
    // pigeons on the street that scatter as you walk through them
    birds: [
      { x: 380, y: 214 }, { x: 398, y: 222 }, { x: 415, y: 210 },
      { x: 880, y: 218 }, { x: 903, y: 226 },
      { x: 1520, y: 212 }, { x: 1542, y: 221 }, { x: 1559, y: 216 },
      { x: 2120, y: 219 }, { x: 2143, y: 211 },
    ],
    // The near-camera layer. Only above the fight and below it - see js/fg.js for why
    // the middle band is deliberately empty.
    fg: [
      { art: 'wires', x: -30, y: -8, top: true },
      { art: 'wires', x: 690, y: -14, top: true, flip: true },
      { art: 'wires', x: 1520, y: -6, top: true },
      { art: 'wires', x: 2280, y: -12, top: true, flip: true },
      { art: 'tarp', x: 400, y: -4, top: true },
      { art: 'garland', x: 1180, y: -6, top: true },
      { art: 'banner', x: 1960, y: -2, top: true },
      { art: 'tarp', x: 2540, y: -6, top: true, flip: true },
      // Standing pieces sit low on purpose. The near depth lane ends at y=241 and the
      // screen at 270, so anything reaching more than about fifteen px above the
      // bottom edge starts hiding a fighter who has walked to the front.
      { art: 'fg_crates', x: 210, y: 312 },
      { art: 'fg_bins', x: 830, y: 300 },
      { art: 'fg_bike', x: 1300, y: 302 },
      { art: 'fg_stall', x: 1780, y: 306 },
      { art: 'fg_crates', x: 2320, y: 310, flip: true },
      { art: 'fg_bins', x: 2620, y: 300 },
      { art: 'wires', x: 2860, y: -10, top: true },
      { art: 'garland', x: 3180, y: -6, top: true },
    ],
    ambience: [
      { kind: 'laundry', x: 210, y: 54, phase: 0 },
      { kind: 'awning', x: 585, y: 86, phase: 17 },
      { kind: 'fan', x: 910, y: 57, phase: 5, alpha: 0.82 },
      { kind: 'laundry', x: 1270, y: 48, phase: 29 },
      { kind: 'awning', x: 1660, y: 88, phase: 41 },
      { kind: 'fan', x: 2010, y: 56, phase: 22, alpha: 0.82 },
      { kind: 'laundry', x: 2360, y: 51, phase: 53 },
      { kind: 'awning', x: 2860, y: 87, phase: 64 },
      { kind: 'fan', x: 3270, y: 58, phase: 31, alpha: 0.82 },
    ],
    emitters: [
      { kind: 'steam', x: 315, y: 162, every: 58, jitter: 34 },
      { kind: 'smoke', x: 982, y: 157, every: 84, jitter: 50 },
      { kind: 'steam', x: 1655, y: 161, every: 62, jitter: 38 },
      { kind: 'smoke', x: 2288, y: 157, every: 92, jitter: 56 },
      { kind: 'steam', x: 3220, y: 160, every: 48, jitter: 30 },
    ],
    waves: [
      { x: 420, spawns: ['goonda', 'goonda', 'bandar', 'masala'] },
      { x: 900, spawns: ['goonda', 'batta', 'masala', 'operator'] },
      { x: 1380, spawns: ['batta', 'bandar', 'goonda', 'goonda'] },
      { x: 1980, spawns: ['pehlwan', 'goonda', 'masala', 'operator', 'batta'] },
      { x: 2580, spawns: ['pehlwan', 'bandar', 'masala', 'goonda', 'sepoy'] },
      { x: 3220, spawns: ['operator', 'masala', 'constable', 'bandar', 'goonda'] },
      { x: 3860, spawns: ['sepoy', 'batta', 'masala', 'goonda', 'constable'] },
      { x: 4520, spawns: ['pehlwan', 'operator', 'constable', 'batta', 'bandar', 'goonda'] },
      { x: 5300, spawns: [], boss: true },
    ],
    build: () => ({
      far: buildDelhiFar(), mid: buildDelhiMid(),
      floor: buildDelhiFloor(),
    }),
    ambient: delhiAmbient,
  },
  {
    id: 'refund', num: '1-3', name: 'REFUND TOWER', sub: 'ACT III - PLEASE HOLD', width: 5300,
    wallKey: 'bg_refund_wall', floorKey: 'bg_refund_floor', floorW: 960,
    music: 'stage3', bossMusic: 'boss3', boss: 'refund',
    lamps: [220, 620, 1020, 1420, 1820, 2220, 2620, 3020], lampCol: '60,220,255', lampA: 0.14,
    rim: '#45e1ee', grade: '45,100,145', gradeA: 0.065, moteCount: 26, moteStyle: 'steam',
    glows: [{ x: 510, y: 140, r: 32 }, { x: 1560, y: 140, r: 34 }, { x: 2700, y: 140, r: 34 }],
    props: [
      { kind: 'table', x: 350, y: 228 }, { kind: 'sign', x: 680, y: 190 },
      { kind: 'crate', x: 1030, y: 202 }, { kind: 'table', x: 1390, y: 228 },
      { kind: 'sign', x: 1760, y: 190 }, { kind: 'crate', x: 2130, y: 202 },
      { kind: 'table', x: 2490, y: 228 }, { kind: 'sign', x: 2850, y: 190 },
    ],
    birds: [],
    fg: [
      { art: 'banner', x: 160, y: -4, top: true }, { art: 'wires', x: 980, y: -10, top: true },
      { art: 'banner', x: 1780, y: -3, top: true, flip: true }, { art: 'wires', x: 2580, y: -11, top: true },
      { art: 'fg_bins', x: 740, y: 301 }, { art: 'fg_crates', x: 2320, y: 311 },
    ],
    ambience: [
      { kind: 'fan', x: 420, y: 57, phase: 3 }, { kind: 'fan', x: 1180, y: 57, phase: 17 },
      { kind: 'fan', x: 1980, y: 57, phase: 31 }, { kind: 'fan', x: 2780, y: 57, phase: 45 },
    ],
    emitters: [{ kind: 'steam', x: 880, y: 160, every: 72, jitter: 35 }, { kind: 'steam', x: 2440, y: 160, every: 68, jitter: 31 }],
    waves: [
      { x: 420, spawns: ['operator', 'goonda', 'operator', 'batta'] },
      { x: 960, spawns: ['operator', 'constable', 'goonda', 'bandar', 'operator'] },
      { x: 1530, spawns: ['sepoy', 'operator', 'goonda', 'constable'] },
      { x: 2160, spawns: ['constable', 'operator', 'pehlwan', 'goonda', 'operator'] },
      { x: 2880, spawns: ['operator', 'operator', 'constable', 'sepoy', 'goonda'] },
      { x: 3500, spawns: ['sepoy', 'operator', 'batta', 'bandar', 'operator'] },
      { x: 4170, spawns: ['pehlwan', 'constable', 'operator', 'goonda', 'batta', 'sepoy'] },
      { x: 5000, spawns: [], boss: true },
    ],
    build: () => ({ far: buildDelhiFar(), mid: buildDelhiMid(), floor: buildDelhiFloor() }), ambient: delhiAmbient,
  },
  {
    id: 'police', num: '1-4', name: 'BLUE LINE LOCKUP', sub: 'ACT IV - POLICE QUARTER', width: 5500,
    wallKey: 'bg_police_wall', floorKey: 'bg_police_floor', floorW: 960,
    music: 'stage1', bossMusic: 'boss', boss: 'yadav',
    lamps: [250, 620, 990, 1370, 1760, 2140, 2550], lampCol: '255,150,72', lampA: 0.12,
    rim: '#6aa8d8', grade: '80,120,170', gradeA: 0.055,
    moteCount: 28, moteStyle: 'dust',
    glows: [{ x: 440, y: 142, r: 28 }, { x: 1390, y: 140, r: 30 }, { x: 2380, y: 142, r: 28 }],
    props: [
      { kind: 'crate', x: 310, y: 204 }, { kind: 'tyres', x: 650, y: 234 },
      { kind: 'sign', x: 930, y: 190 }, { kind: 'table', x: 1210, y: 228 },
      { kind: 'crate', x: 1530, y: 202 }, { kind: 'tyres', x: 1840, y: 235 },
      { kind: 'sign', x: 2190, y: 191 }, { kind: 'crate', x: 2510, y: 202 },
    ],
    birds: [{ x: 520, y: 218 }, { x: 542, y: 225 }, { x: 1720, y: 220 }],
    fg: [
      { art: 'wires', x: 20, y: -10, top: true }, { art: 'banner', x: 790, y: -4, top: true },
      { art: 'wires', x: 1500, y: -12, top: true, flip: true }, { art: 'tarp', x: 2320, y: -5, top: true },
      { art: 'fg_bins', x: 720, y: 300 }, { art: 'fg_bike', x: 1680, y: 304 },
      { art: 'fg_crates', x: 2600, y: 310, flip: true },
    ],
    ambience: [
      { kind: 'fan', x: 660, y: 58, phase: 3, alpha: 0.82 },
      { kind: 'awning', x: 1060, y: 88, phase: 21, alpha: 0.78 },
      { kind: 'fan', x: 1570, y: 56, phase: 15, alpha: 0.84 },
      { kind: 'fan', x: 2500, y: 58, phase: 29, alpha: 0.84 },
    ],
    emitters: [
      { kind: 'smoke', x: 380, y: 158, every: 88, jitter: 45 },
      { kind: 'steam', x: 1320, y: 163, every: 68, jitter: 38 },
      { kind: 'smoke', x: 2290, y: 159, every: 90, jitter: 48 },
    ],
    waves: [
      { x: 420, spawns: ['goonda', 'batta', 'goonda'] },
      { x: 960, spawns: ['masala', 'goonda', 'pehlwan'] },
      { x: 1510, spawns: ['batta', 'batta', 'bandar', 'goonda'] },
      { x: 2100, spawns: ['pehlwan', 'masala', 'goonda', 'batta'] },
      { x: 2640, spawns: ['constable', 'constable', 'sepoy', 'operator', 'goonda'] },
      { x: 3300, spawns: ['constable', 'operator', 'sepoy', 'batta', 'goonda'] },
      { x: 4000, spawns: ['sepoy', 'constable', 'bandar', 'operator', 'pehlwan'] },
      { x: 4680, spawns: ['constable', 'constable', 'sepoy', 'batta', 'operator', 'goonda'] },
      { x: 5200, spawns: [], boss: true },
    ],
    build: () => ({ far: buildDelhiFar(), mid: buildDelhiMid(), floor: buildDelhiFloor() }),
    ambient: delhiAmbient,
  },
  {
    id: 'fort', num: '1-5', name: 'IRON MONSOON', sub: 'ACT V - THE RED FORT ARSENAL', width: 6100,
    wallKey: 'bg_fort_wall', floorKey: 'bg_fort_floor', floorW: 960,
    music: 'boss', bossMusic: 'boss', boss: 'rana',
    lamps: [190, 560, 940, 1320, 1710, 2090, 2500], lampCol: '255,90,52', lampA: 0.14,
    rim: '#bf5668', grade: '155,46,58', gradeA: 0.06,
    moteCount: 36, moteStyle: 'dust',
    glows: [{ x: 320, y: 150, r: 35 }, { x: 1260, y: 150, r: 38 }, { x: 2220, y: 150, r: 38 }],
    props: [
      { kind: 'crate', x: 280, y: 202 }, { kind: 'crate', x: 330, y: 232 },
      { kind: 'tyres', x: 720, y: 234 }, { kind: 'table', x: 1050, y: 228 },
      { kind: 'crate', x: 1420, y: 200 }, { kind: 'sign', x: 1770, y: 190 },
      { kind: 'tyres', x: 2130, y: 235 }, { kind: 'crate', x: 2490, y: 201 },
    ],
    birds: [],
    fg: [
      { art: 'banner', x: 120, y: -4, top: true }, { art: 'wires', x: 860, y: -12, top: true },
      { art: 'banner', x: 1590, y: -2, top: true, flip: true }, { art: 'wires', x: 2300, y: -10, top: true },
      { art: 'fg_crates', x: 470, y: 311 }, { art: 'fg_bins', x: 1350, y: 302 },
      { art: 'fg_crates', x: 2440, y: 311, flip: true },
    ],
    ambience: [
      { kind: 'awning', x: 410, y: 86, phase: 4, alpha: 0.78 },
      { kind: 'laundry', x: 1140, y: 50, phase: 19, alpha: 0.76 },
      { kind: 'awning', x: 1900, y: 87, phase: 37, alpha: 0.78 },
      { kind: 'laundry', x: 2550, y: 48, phase: 55, alpha: 0.76 },
    ],
    emitters: [
      { kind: 'smoke', x: 360, y: 162, every: 58, jitter: 34 },
      { kind: 'smoke', x: 1310, y: 160, every: 62, jitter: 36 },
      { kind: 'steam', x: 2220, y: 162, every: 56, jitter: 32 },
    ],
    waves: [
      { x: 390, spawns: ['pehlwan', 'goonda', 'goonda'] },
      { x: 900, spawns: ['masala', 'batta', 'bandar', 'goonda'] },
      { x: 1450, spawns: ['pehlwan', 'pehlwan', 'batta'] },
      { x: 2050, spawns: ['masala', 'goonda', 'batta', 'pehlwan'] },
      { x: 2600, spawns: ['sepoy', 'sepoy', 'constable', 'pehlwan', 'operator', 'batta'] },
      { x: 3360, spawns: ['sepoy', 'operator', 'constable', 'batta', 'goonda', 'bandar'] },
      { x: 4100, spawns: ['sepoy', 'sepoy', 'constable', 'masala', 'pehlwan'] },
      { x: 4850, spawns: ['pehlwan', 'sepoy', 'operator', 'batta', 'constable', 'goonda'] },
      { x: 5750, spawns: [], boss: true },
    ],
    build: () => ({ far: buildDelhiFar(), mid: buildDelhiMid(), floor: buildDelhiFloor() }),
    ambient: delhiAmbient,
  },
