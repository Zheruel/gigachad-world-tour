// delhi_bosses.js - the three DIRTY DELHI fights. Each one is a set of hooks the shared
...
// damage: Pappu's ring, Langda's wire and stolen meter, the dredger's winch and cab.
import { G, W, FLOOR_TOP, FLOOR_BOT, clamp, irand, diff, arenaMin, arenaMax, laneMin, fall, inAir, addMeter } from './engine.js';
...
// ============================================================ LANGDA
// He does not stand and trade. He runs the wire over the goods market, comes down on
// a timer whatever you do, steals a segment of meter, and goes back up. Three ways to
// reach him: counter the drop, jump-kick him on the wire, or break the brackets.
// The wire is a wall-plane thing at one screen height, strung across the awnings. He
// tracks the player's lane underneath it, so his height above his own lane is whatever
// puts his hands (the top of the 103-tall hang frame) on the wire.
const WIRE_Y = 70;
const wireZ = (y) => y - WIRE_Y - 99;
const GROUND_T = 240;       // how long he stays down after a drop before climbing
const DROP_EVERY = 540;     // the guard: 9 seconds, and everything clever makes it sooner
const LANGDA_WIND = { snatch: 12, screech: 26, throw: 10, troop: 8, lunge: 12, drop: 40 };

function brackets() {
  return G.props.filter((q) => q.prop === 'bracket' && !q.broken);
}
function wireSpan() {
  const list = brackets();
  if (list.length < 2) return null;
  let lo = 1e9, hi = -1e9;
  for (const q of list) { lo = Math.min(lo, q.x); hi = Math.max(hi, q.x); }
  return { lo: lo + 4, hi: hi - 4 };
}

function langdaReturnMeter(b) {
  if (!b.stolen) return;
  addMeter(b.stolen);
  spawnPop(b.x, b.y - b.z - 70, 'METER BACK');
  b.stolen = 0;
}

const langda = {
  init(b) {
    b.stolen = 0; b.sinceDrop = 0; b.groundT = 0; b.troopCd = 300;
    b.onWire = true; b.z = wireZ(b.y); b.wireGone = !wireSpan(); b.state = 'wire'; b.atkCd = 60;
    if (b.wireGone) { b.onWire = false; b.z = 0; b.state = 'idle'; }
    b.parried = (dmg, dir) => {
      // the drop, countered: the enormous punish
      b.hurt(16, dir, true, true);
      if (!b.dead) { b.state = 'down'; b.t = 0; b.vx = dir * 1.4; b.vz = 2.4; b.z = Math.max(b.z, 0.1); b.groundT = GROUND_T; b.onWire = false; }
      langdaReturnMeter(b);
    };
  },
  intro(b, t) {
    // he drops onto the wire from the awnings above the screen
    b.onWire = true;
    b.z = t < 30 ? 240 : Math.max(wireZ(b.y), 240 - (t - 30) * 6);
    if (t > 90 && t < 130) b.pattern = 'screech';
  },
  update(b) {
    const p = G.player;
    const spd = bossSpeed(b);
    const span = wireSpan();
    if (!span && !b.wireGone) { b.wireGone = true; say(b, 'NO WIRE'); }
    if (b.onWire) {
      b.sinceDrop++;
      if (b.wireGone && b.state !== 'drop' && b.state !== 'down') {
        // the last anchor went while he was up there: he comes down on his back
        b.onWire = false; b.state = 'down'; b.t = 0; b.vz = 0.5; b.groundT = GROUND_T;
        b.hurt(10, -b.face, true, true);
        langdaReturnMeter(b);
        return true;
      }
    } else if (b.groundT > 0 && b.state !== 'down' && b.state !== 'dying') b.groundT--;
    if (b.troopCd > 0) b.troopCd--;

    switch (b.state) {
      case 'wire': {
        // run the wire: hover a little off the player's x, never directly over him unless
        // the drop is coming
        const wantX = clamp(p.x + (p.x < b.x ? 56 : -56), span.lo, span.hi);
        b.x += clamp(wantX - b.x, -spd, spd);
        b.y += clamp(clamp(p.y, laneMin(b.x), FLOOR_BOT) - b.y, -1.2, 1.2);
        b.z = wireZ(b.y);
        if (--b.atkCd <= 0) {
          const opts = ['throw', 'throw'];
          if (b.troopCd <= 0 && aliveEnemies() < 2) opts.push('troop', 'troop');
          if (b.sinceDrop > 240) opts.push('drop', 'drop');
          b.pattern = b.sinceDrop > DROP_EVERY ? 'drop' : opts[irand(0, opts.length - 1)];
          b.state = 'windup'; b.t = 0; b.hitLanded = false;
          if (b.pattern === 'drop') b.state = 'dropaim';
        }
        return true;
      }
      case 'dropaim': {
        // 40 frames over your shadow, green: the only time he is at your height is next
        b.pattern = 'drop';
        b.x += clamp(p.x - b.x, -2.4, 2.4);
        b.y += clamp(clamp(p.y, laneMin(b.x), FLOOR_BOT) - b.y, -1.6, 1.6);
        b.z = wireZ(b.y);
        if (b.t >= LANGDA_WIND.drop) { b.state = 'drop'; b.t = 0; b.hitLanded = false; G.audio.sfx('jump'); }
        return true;
      }
      case 'drop': {
        b.z -= 4.5;
        b.x += clamp(p.x - b.x, -1.2, 1.2);
        if (b.z <= 0) {
          b.z = 0; b.onWire = false; b.sinceDrop = 0; b.groundT = GROUND_T;
          spawnDust(b.x, b.y, 4); G.shake = Math.max(G.shake, 5);
          if (Math.abs(p.x - b.x) < 30 && Math.abs(p.y - b.y) < 16 && p.z < 20 && !p.dying
              && p.state !== 'down' && p.state !== 'getup') {
            if (!resolveIncomingHit(p, b, { parryClass: 'counter' })) {
              hurtPlayer(p, 14, p.x < b.x ? -1 : 1, true);
              spawnSpark(p.x, p.y - 40);
              G.audio.sfx('heavy');
            } else return true;   // b.parried() put him down
          }
          hitEnemiesNear(b.x, b.y, 30, 16, 8, undefined, true, null);
          b.state = 'land'; b.t = 0;
        }
        return true;
      }
      case 'land': {
        if (b.t > 22) { b.state = 'idle'; b.atkCd = 20; }
        return true;
      }
      case 'idle': {
        // on the ground: a small furious animal
        b.z = 0;
        approach(b, spd * 1.1, 20, 36);
        if (b.groundT <= 0 && !b.wireGone && !b.enraged) { b.state = 'climb'; b.t = 0; return true; }
        if (--b.atkCd <= 0) {
          const d = Math.abs(p.x - b.x);
          const opts = [];
          if (d < 70) opts.push('snatch', 'snatch');
          if (d < 170) opts.push('screech');
          if (b.enraged) opts.push('lunge', 'lunge');
          if (d >= 70) opts.push('lunge');
          b.pattern = opts.length ? opts[irand(0, opts.length - 1)] : 'screech';
          if (Math.random() < 0.3) say(b, b.def.lines[irand(0, b.def.lines.length - 1)]);
          b.state = 'windup'; b.t = 0; b.hitLanded = false;
        }
        return true;
      }
      case 'windup': {
        if (b.t >= (LANGDA_WIND[b.pattern] || 12)) {
          b.state = b.pattern; b.t = 0; b.hitLanded = false;
          if (b.pattern === 'lunge') { b.vx = b.face * 3.2; G.audio.sfx('dash'); }
          if (b.pattern === 'screech') G.audio.sfx('enrage');
        }
        return true;
      }
      case 'throw': {
        // a brick off the wire, green: reflect it and it knocks him off
        if (b.t === 6) {
          const dx = clamp((p.x - b.x) / 40, -3.4, 3.4);
          spawnArc('brick', b.x, b.y, dx, 0.4, 8, null, { source: b, parryClass: 'reflect', z: b.z + 60 });
          G.audio.sfx('throw');
        }
        if (b.t > 26) { b.state = b.onWire ? 'wire' : 'idle'; b.atkCd = irand(50, 90) * cdScale(b); }
        return true;
      }
      case 'troop': {
        if (b.t === 6) {
          b.troopCd = 1200;
          spawnEnemy('bandar', G.camX - 20, laneMin(G.camX) + 16);
          spawnEnemy('bandar', G.camX + W + 20, FLOOR_BOT - 12);
          spawnPop(b.x, b.y - b.z - 70, 'THE TROOP');
          G.audio.sfx('blip');
        }
        if (b.t > 30) { b.state = b.onWire ? 'wire' : 'idle'; b.atkCd = irand(40, 80) * cdScale(b); }
        return true;
      }
      case 'snatch': {
        // the grab: a pickup off the floor, or a segment of your meter, and back up
        if (b.t < 10) b.x += b.face * 2.6;
        if (b.t === 10 && !b.hitLanded) {
          b.hitLanded = true;
          const canTake = Math.abs(p.x - b.x) < 34 && Math.abs(p.y - b.y) < 16 && p.z < 12
            && !p.dying && p.state !== 'down' && p.state !== 'getup' && p.invuln <= 0;
          if (canTake) {
            const take = Math.min(G.meter, 25);
            hurtPlayer(p, 5, b.face, false);
            if (take > 0) {
              G.meter -= take; b.stolen += take;
              spawnPop(p.x, p.y - 100, 'METER SNATCHED');
              G.audio.sfx('blip');
            }
            b.groundT = 0;   // and straight back up with it
          } else {
            const q = G.pickups.find((k) => k.kind !== 'life' && Math.abs(k.x - b.x) < 40 && Math.abs(k.y - b.y) < 14);
            if (q) { G.pickups.splice(G.pickups.indexOf(q), 1); spawnPop(b.x, b.y - 60, 'MINE'); b.groundT = 0; }
          }
        }
        if (b.t > 28) { b.state = 'idle'; b.atkCd = irand(30, 70) * cdScale(b); }
        return true;
      }
      case 'screech': {
        // a full-lane shriek that staggers everything on the ground, his own troop included
        if (b.t === 2) {
          G.shake = Math.max(G.shake, 6);
          spawnRing(b.x, b.y - 30, '#ffd0a0');
          reactStage(b.x, 2);
          if (Math.abs(p.x - b.x) < 190 && Math.abs(p.y - b.y) < 40 && p.z < 18 && !p.dying
              && p.state !== 'down' && p.state !== 'getup') {
            hurtPlayer(p, 6, p.x < b.x ? -1 : 1, false);
          }
          for (const e of G.enemies) {
            if (e.dead || e.state === 'dying' || Math.abs(e.x - b.x) > 190 || e.z > 18) continue;
            e.hurt(4, e.x < b.x ? -1 : 1, false, false);
            if (!e.dead) { e.state = 'stagger'; e.t = 0; e.vx = (e.x < b.x ? -1 : 1) * 0.8; }
          }
        }
        if (b.t > 40) { b.state = 'idle'; b.atkCd = irand(70, 120) * cdScale(b); }
        return true;
      }
      case 'lunge': {
        b.x += b.vx; b.vx *= 0.92;
        if (b.t === 8 && !b.hitLanded) { b.hitLanded = true; tryHitPlayer(b, 9, 40, false, 15, 'counter'); }
        if (b.t > 22) { b.vx = 0; b.state = 'idle'; b.atkCd = irand(30, 60) * cdScale(b); }
        return true;
      }
      case 'climb': {
        // back up the nearest bracket
        const list = brackets();
        if (!list.length || b.wireGone) { b.state = 'idle'; b.atkCd = 20; return true; }
        let best = list[0];
        for (const q of list) if (Math.abs(q.x - b.x) < Math.abs(best.x - b.x)) best = q;
        if (Math.abs(best.x - b.x) > 6 && b.z <= 0) {
          b.x += Math.sign(best.x - b.x) * spd * 1.2;
          b.face = best.x < b.x ? -1 : 1;
        } else {
          const top = wireZ(b.y);
          b.z = Math.min(top, b.z + 1.6);
          if (b.z >= top) { b.onWire = true; b.state = 'wire'; b.atkCd = irand(30, 70); b.sinceDrop = Math.max(b.sinceDrop, 120); }
        }
        return true;
      }
      case 'recover': case 'hurt': case 'stagger': case 'down': case 'dying': return false;
      case 'stagger_wire': return false;
    }
    return false;
  },
  onHurt(b, dmg, heavy, launch) {
    // a reflected brick, or a jump kick with enough behind it, knocks him off the wire
    if (b.onWire && (heavy || launch)) {
      b.onWire = false; b.state = 'down'; b.t = 0; b.vz = 1.5; b.vx = 0; b.groundT = GROUND_T; b.sinceDrop = 0;
      langdaReturnMeter(b);
      return true;
    }
    if (b.onWire) { b.flash = 5; return true; }   // a glancing hit: he hangs on
    return false;
  },
  onDown(b) { langdaReturnMeter(b); },
  onEnrage(b) {
    // brackets down, troop dead, and the last third is a small furious animal in an
    // open street with nowhere to go
    for (const q of brackets()) q.hurt(999, 1);
    b.wireGone = true;
    if (b.onWire) { b.onWire = false; b.state = 'down'; b.t = 0; b.vz = 0.5; b.groundT = 0; langdaReturnMeter(b); }
  },
  onDeath(b) { langdaReturnMeter(b); },
  frame(b) {
    const hang = (G.time >> 3) % 3;
    switch (b.state) {
      case 'wire': return ['hang', b.moved > 0.3 ? hang : 0];
      case 'dropaim': return ['hang', 2];
      case 'drop': return ['drop', b.t < 6 ? 1 : 2];
      case 'land': return ['drop', 3];
      case 'climb': return b.z > 0 ? ['hang', hang] : ['idle', (G.time >> 3) & 3];
      case 'idle': return ['idle', (G.time >> (b.moved > 0.2 ? 2 : 4)) & 3];
      case 'windup':
        if (b.onWire) return ['hang', b.pattern === 'throw' ? 2 : 1];   // one hand on the wire
        return b.pattern === 'snatch' ? ['snatch', 0] : b.pattern === 'screech' ? ['screech', b.t < 12 ? 0 : 1]
          : b.pattern === 'throw' ? ['throw', 0] : b.pattern === 'troop' ? ['screech', 1] : ['idle', 0];
      case 'snatch': return ['snatch', b.t < 12 ? 1 : 2];
      case 'screech': return ['screech', b.t < 20 ? 2 : 3];
      case 'throw': return b.onWire ? ['hang', b.t < 8 ? 2 : 1] : ['throw', b.t < 8 ? 1 : 2];
      case 'troop': return b.onWire ? ['hang', (b.t >> 2) & 1 ? 1 : 2] : ['screech', 2];
      case 'lunge': return ['drop', 2];
      case 'recover': return ['snatch', 2];
      case 'hurt': return ['hurt', b.t < 5 ? 1 : 0];
      case 'stagger': return ['hurt', (b.t >> 3) & 1];
      case 'down': case 'dying': return ['down', 0];
    }
  ...
  draw(ctx, b, camX) {
    const [name, idx] = langda.frame(b);
    const f = getFrame(b.set, name, idx, b.face);
    const sx = Math.round(b.x - camX), sy = Math.round(b.y - b.z);
    const dx = sx - Math.round(frameW(f) / 2), dy = sy - frameH(f) + 4;
    const cue = (b.state === 'windup' && b.t > 4) || (b.state === 'dropaim' && b.t > 10);
    blitTelegraph(ctx, b, f, dx, dy, cue);
    if (cue) drawCueMarker(ctx, b, sx, sy - frameH(f) - 4);
    // what he stole, on the string round his neck
    if (b.stolen > 0 && b.state !== 'dying') {
      const bx = sx - 6, by = sy - frameH(f) * 0.55;
      ctx.fillStyle = '#100a0c'; ctx.fillRect(bx - 1, by - 1, 14, 5);
      ctx.fillStyle = '#ffd94a'; ctx.fillRect(bx, by, Math.round(12 * b.stolen / 25), 3);
      if ((G.rawTime >> 3) & 1) { ctx.fillStyle = '#fff4d0'; ctx.fillRect(bx, by, 12, 1); }
    }
  },
  // The wire itself, strung between whatever brackets are still up. Drawn on the wall
  // plane by stages.js so he hangs in front of it.
  drawWire(ctx, camX) {
    const list = brackets().sort((a, c) => a.x - c.x);
    if (list.length < 2) return;
    ctx.save();
    ctx.lineWidth = 2;
    for (let i = 0; i < list.length - 1; i++) {
      const a = list[i], c = list[i + 1];
      const ax = a.x - camX, cx = c.x - camX;
      const sag = 7 + Math.sin(G.rawTime * 0.05 + i) * 1.2;
      // the cable and a thin highlight along its top, so it reads against the awnings
      ctx.strokeStyle = 'rgba(12,8,8,0.95)';
      ctx.beginPath(); ctx.moveTo(ax, WIRE_Y); ctx.quadraticCurveTo((ax + cx) / 2, WIRE_Y + sag, cx, WIRE_Y); ctx.stroke();
      ctx.strokeStyle = 'rgba(200,190,170,0.45)'; ctx.lineWidth = 1;
      ctx.beginPath(); ctx.moveTo(ax, WIRE_Y - 1); ctx.quadraticCurveTo((ax + cx) / 2, WIRE_Y + sag - 1, cx, WIRE_Y - 1); ctx.stroke();
      ctx.lineWidth = 2;
    }
    ctx.restore();
  },
...
export const DELHI = { pappu, langda, dredger };
...
// Something for the wall plane: the wire Langda runs. stages.js calls this so the cable
// sits behind him and behind the fighters.
export function drawDelhiWallPlane(ctx, camX) {
  const b = G.boss;
  if (!b || b.removeMe) return;
  if (b.key === 'langda' && !b.wireGone) langda.drawWire(ctx, camX);
}
