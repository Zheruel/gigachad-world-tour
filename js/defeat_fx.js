...
export function spawnDefeatFX(e,dir,heavy,launch){
 ...
 const seed=(Math.round(e.x*31)^Math.round(e.y*17)^Math.imul(G.rawTime|0,131))>>>0;
 const animal=ANIMALS.has(e.kind)||e.pitCd>0,strong=!!(heavy||launch||e.superApplying);
 const superHit=e.superApplying&&G.player?.specialTarget===e,superT=G.player?.superT;
 const upper=superHit&&(superT===76||G.player.superGuarded&&superT===56);
 const x=upper?G.player.x+G.player.face*14:e.x;
 const y=e.y-Math.max(0,e.z)-(upper?75:superHit&&superT===60?65:(e.h||80)*.55);
 if(animal){
