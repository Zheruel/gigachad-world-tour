#!/usr/bin/env python3
"""Build V6 from one combined CHAD + cigar + motorcycle master sheet."""
...
SOURCE = ROOT / "assets" / "ai" / "entrance_v6" / "combined_master_sheet.png"
OUTPUT = ROOT / "assets" / "story" / "entrance_v6"
CANVAS = (277, 186)
GROUND = 181
TARGET_MAX_HEIGHT = 142
ROW_EDGES = [0, 330, 640, 1024]
...
def keep_main_rig(image: Image.Image) -> Image.Image:
    """Drop isolated key-removal flecks without separating cigar from face."""
...
def sheet_cells(sheet: Image.Image) -> list[Image.Image]:
    ...
    for row in range(3):
        for column in range(4):
            ...
            result.append(sheet.crop((x0, ROW_EDGES[row], x1, ROW_EDGES[row + 1])))
    return result
...
def build() -> None:
    ...
    contact = Image.new("RGBA", (CANVAS[0] * 4, CANVAS[1] * 3), (21, 17, 25, 255))
    ...
    print(f"Built {len(frames)} combined V6 frames at shared scale {scale:.4f}")
