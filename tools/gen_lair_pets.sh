...
$G $O/baitfish.png landscape "One horizontal row containing a four frame swim cycle of the same fish, evenly spaced, identical scale, all on the same horizontal line, with a clear band of empty flat green between each pose so they never touch. The fish is identical in every pose: same body, same markings, same colour. Left to right the four poses are: 1 tail swung fully left and the body curved, 2 tail passing through the centre and the body straight, 3 tail swung fully right and the body curved the other way, 4 tail passing back through the centre. The head stays at exactly the same height and never rotates; only the tail, the body curve and the fins change. The fish is a fat healthy mackerel seen in exact side profile facing right, drawn LARGE and filling most of the height of each cell, richly detailed: a deep blue-green back crossed by bold dark wavy tiger bars, a bright mirror-silver flank below them with a faint gold sheen along the middle, a white belly, one clear round black eye with a pale ring, a visible gill plate, a small spiny dorsal fin, paired pectoral fins and a deeply forked tail. $SHEET" &
wait
