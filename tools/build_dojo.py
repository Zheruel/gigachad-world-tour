#!/usr/bin/env python3
"""Split the dojo hub plate into wall and floor assets.
...
p.add_argument("src", nargs="?", default="assets/ai/dojo/room.png")
...
wall.quantize(colors=128, method=Image.Quantize.MEDIANCUT,
              dither=Image.Dither.NONE).convert("RGB").save("assets/bg_dojo_wall.png")
floor.quantize(colors=72, method=Image.Quantize.MEDIANCUT,
               dither=Image.Dither.NONE).convert("RGB").save("assets/bg_dojo_floor.png")
print(f"dojo: seam {sy}/{h}, wall {WIDTH}x{WALL_H}, floor {WIDTH}x{FLOOR_H}")
