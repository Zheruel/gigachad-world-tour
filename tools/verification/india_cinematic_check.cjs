...
(async()=>{const browser=await chromium.launch({channel:'chrome',headless:true});try{
 ...
 const checks=await page.evaluate(async()=>{
  ...
  g.indiaScene('delhi','intro',0);ok('market opening lasts540ticks',G.stage.introTicks===540);updateIndiaIntro(251);ok('barricade intact before impact',!G.india.marketBroken);updateIndiaIntro(252);ok('barricade breaks at impact',G.india.marketBroken);updateIndiaIntro(540);ok('market entrance reaches first encounter without teleport',G.player.x===348&&G.player.y===236);
  ...
  g.indiaScene('delhi','intro',0);G.waveIndex=-1;G.stage.waves.forEach(w=>w.done=false);g.step(542);g.press('right');g.step(3);g.release('right');ok('market returns to first wave without a second generic quote',G.waveIndex===0&&sounds.filter(n=>n==='duke_who_wants_some').length===1&&!sounds.includes('duke_come_get_some'));
