...
(async()=>{const b=await chromium.launch({channel:'chrome',headless:true});try{
 ...
 await p.keyboard.press('KeyQ');await p.waitForFunction(()=>__game.G.audio.has('duke_time_to_crash_this_party')&&__game.G.audio.has('duke_checks_cash'));
 const checks=await p.evaluate(async()=>{
  ...
  G.audio.stopSamples();G.audio.voice('duke_time_to_crash_this_party',2155,true);G.audio.voice('duke_checks_cash',3330,true);
  ...
  G.audio.roomSfx('room_page',.2);G.audio.voice('duke_checks_cash',3330,true);
