...
function moments(){const scene=$('scene').value,list=scene==='intro'&&stage==='refund'?[[0,'Callers at work'],[103,'Wall cracks'],[120,'Shoulder breach'],[210,'Debris settles'],[300,'Guard'],[479,'Ready']]:beats[scene]||[];$('moment').replaceChildren(new Option('Performance beat…',''));for(const [t,label]of list)$('moment').add(new Option(`${t}: ${label}`,t));}
...
stageOptions();
for(const option of [...$('pattern').options])if(option.value&&!['guardbreak','damaged',...(stage==='delhi'?['ladle','utensil','rush','valve']:['boxing','handset','shove','call'])].includes(option.value))option.remove();
