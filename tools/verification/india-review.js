...
let game,manifest,sprites,combat,playing=false,mode='scene',poseTick=0,effectPreset=null;
const query=new URLSearchParams(location.search);$('stage').value=query.get('stage')==='refund'?'refund':'delhi';$('time').value=query.get('t')||0;
function stageOptions(){const id=$('stage').value;$('scene').replaceChildren();for(const s of ['card','intro',...areas[id],...(id==='delhi'?['dredger']:['finish']),'clear'])$('scene').add(new Option(s.replaceAll('_',' '),s));$('scene').value=query.get('scene')||areas[id][0];if(!$('scene').value)$('scene').value=areas[id][0];}
stageOptions();
function stop(){playing=false;$('play').textContent='Play';}
function layers(){game.G.india.review=Object.fromEntries(['environment','ambient','props','fx'].map(k=>[k,$(k).checked]));}
function drawActors(){
 if(!sprites||!manifest)return;const key=$('character').value,state=$('action').value,frames=manifest[key]?.[state]||[],n=Math.max(frames.length,1),index=Math.floor(poseTick/8)%n;
...
function character(){$('action').replaceChildren();for(const state of Object.keys(manifest[$('character').value]||{}))$('action').add(new Option(state,state));poseTick=0;drawActors();}
{const timer=setInterval(async()=>{game=frame.contentWindow.__game;if(!game||game.G.state==='boot')return;clearInterval(timer);manifest=await(await fetch('./assets/frames/manifest.json')).json();sprites=await frame.contentWindow.eval(`import(${JSON.stringify(new URL('./js/sprites.js',frame.contentWindow.location.href).href)})`);combat=await frame.contentWindow.eval(`import(${JSON.stringify(new URL('./js/player.js',frame.contentWindow.location.href).href)})`);for(const key of Object.keys(manifest).filter(k=>k==='player'||k.startsWith('ic_')))$('character').add(new Option(key,key));character();seek();},100);}
$('stage').onchange=()=>{effectPreset=null;stageOptions();$('time').value=0;seek()};$('scene').onchange=()=>{effectPreset=null;$('time').value=0;seek()};$('time').oninput=seek;
...
for(const id of ['environment','ambient','props','fx'])$(id).onchange=draw;
