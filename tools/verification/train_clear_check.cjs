...
(async()=>{const browser=await chromium.launch({channel:'chrome',headless:true});try{
 ...
 const checks=await f.evaluate(()=>{const g=__game,G=g.G,out=[],ok=(n,v)=>out.push([n,!!v]);
  ...
  g.press('use');g.step(1);g.release('use');g.step(35);ok('fresh F continues to silent Delhi loading card',G.state==='chapter-card'&&G.stage.id==='delhi'&&G.audio.snapshot().music===null);
  g.press('use');g.step(1);g.release('use');ok('separate fresh F starts Delhi intro and music',G.state==='intro'&&G.audio.snapshot().music==='stage1a');
  ...
  g.trainScene('escape',1259);const pad={connected:true,axes:[0,0,0,0],buttons:Array.from({length:16},()=>({pressed:false,value:0}))};pad.buttons[4]={pressed:true,value:1};Object.defineProperty(navigator,'getGamepads',{configurable:true,value:()=>[pad]});g.step(240);ok('held LB cannot dismiss tally',G.state==='clear');pad.buttons[4]={pressed:false,value:0};g.step(2);pad.buttons[4]={pressed:true,value:1};g.step(1);pad.buttons[4]={pressed:false,value:0};g.step(35);ok('fresh LB continues to silent Delhi loading card',G.state==='chapter-card'&&G.stage.id==='delhi'&&G.audio.snapshot().music===null);pad.buttons[4]={pressed:true,value:1};g.step(1);pad.buttons[4]={pressed:false,value:0};ok('separate fresh LB starts Delhi intro',G.state==='intro');Object.defineProperty(navigator,'getGamepads',{configurable:true,value:()=>[]});g.resetInput();
  return out;
 });
