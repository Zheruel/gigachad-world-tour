...
R=assets/sources/production/ref_chad.png
O=assets/sources/production/lair_room5
S=assets/sources/production/lair_sky
mkdir -p $O $S
...
echo "=== panel D: the master bedroom ==="
# The room got a fourth screen. The bedroom is the same walnut and brass as the lounge -
# a black lacquer boudoir would have read as a different apartment - and it ends in a
# corner window, which is why rebuild_window() in build_lair_wide.py has to cope with
# more than one opening in the plate.
$G $O/panel_d.png landscape "The right hand end of the private penthouse of a gigachad brawler at the top of a skyscraper, his master bedroom, seen flat side on and completely empty. $STYLE It continues the room in the reference image exactly: same ceiling beam at the same height, same neon strips, same floor seam at the same height, same walnut and brass. Across the wall, left to right: from 0 to 12 percent of the width a tall open doorway through the walnut panelling into this room, its architrave in walnut with polished brass edging and a warm light spilling through it; from 14 to 36 percent a grand carved white marble fireplace set into the walnut with a brass fender and a dark empty firebox, a heavy gilt overmantel mirror above it; from 38 to 50 percent plain blank walnut panelling with a single brass picture light mounted high and nothing at all hanging beneath it; from 52 to 100 percent an enormous floor to ceiling corner window, nearly half the width of the whole image, in slim black steel mullions running all the way to the right hand edge, and the glass of that window is SOLID FLAT BRIGHT GREEN, pure chroma key green RGB 0 255 0, completely featureless, with only the black steel window frames and mullions drawn on top of it. There is no view, no city, no sky and no reflection in the window: it is flat green. $RULES $EMPTY. $BG" "$O/panel_c.png" "$R"
...
# NEAR is two panels now. At 0.42 parallax across a 1920-wide room the camera drags it
# 605 logical px, so a single 480-wide plate would visibly repeat inside one pan.
$G $S/near_b.png landscape "The tops of a handful of large city buildings seen from high up and very close to the viewer, standing along the very BOTTOM edge of the frame. Their roofs reach only about one third of the way up the image and no higher. They are near-black silhouettes with hard edges, richly detailed along their roof lines with a rooftop helipad and its circle marking, satellite dishes, cooling towers, a crane, stair bulkheads, handrails and cable runs, tiny red aircraft warning lights, and a couple of big horizontal neon signs glowing cyan and gold along their parapets. Their upper faces catch a thin rim of warm sunset light along one edge. The ENTIRE upper two thirds of the image, everything above the roof lines, is SOLID FLAT BRIGHT GREEN, pure chroma key green RGB 0 255 0, completely featureless: no sky, no clouds, no haze, no colour, no buildings. $CITY" &
...
echo "=== now: ./.venv/bin/python tools/production/build_lair_wide.py ==="
