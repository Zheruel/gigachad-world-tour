...
# or be redrawn. tools/production/build_lair_extras.py then only has to take out translation, and
...
O=assets/sources/production/lair/bed
mkdir -p $O
...
echo "  ./.venv/bin/python tools/verification/check_bed_poses.py"
ls -la $O
