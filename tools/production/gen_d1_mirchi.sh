...
# Usage: tools/production/gen_d1_mirchi.sh [refs|sheets|cart|all]
set -uo pipefail
cd "$(dirname "$0")/../.."
G=tools/production/gen_codex.sh
O=assets/sources/production/stages/dirty_delhi/cast
mkdir -p $O
WHAT="${1:-all}"
...
$G assets/sources/production/stages/dirty_delhi/props/mirchicart.png landscape "$CART" &
$G assets/sources/production/stages/dirty_delhi/props/mirchicart_b.png landscape "Solid flat bright green chroma-key background (RGB 0,255,0) filling the entire image behind the subject, nothing else in the background. The same cart as the reference, wrecked: tipped over onto its side with both wheels in the air and one wheel bent, the awning collapsed and torn, jars smashed, golgappa puris scattered on the ground, the kadai upended, the MIRCHI CHAAT sign cracked in half. $CART" "assets/sources/production/stages/dirty_delhi/props/mirchicart.png" &
wait
}
