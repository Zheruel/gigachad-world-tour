...
#
# The gym is not here: tools/production/gen_lair_gym.sh owns everything in front of the glass,
...
# likes (3.8 and 4.3 against the shelf's 5.0), so build_bar() in tools/production/build_lair_extras.py
...
# drawn mirrored (d.flip in js/hub.js) so the pair does not read as copy-paste.
#
# Pass the SOFA as a reference image. Described in words alone it came back as a subtly
# different suite - a different black, chunkier arms, a different buttoning scale - which is
# exactly the thing you notice when the two are side by side and cannot name.
$G $O/lounge_chair.png square "The matching ARMCHAIR to the black leather chesterfield sofa in the attached reference image. It is the same suite: exactly the same black leather, exactly the same deep diamond buttoning at the same scale, exactly the same line of brass studs, exactly the same rolled arm profile and arm height, exactly the same polished chrome ball feet, exactly the same viewing angle and the same key light from the top left. It is simply one seat wide instead of three. Nobody is sitting in it and there is nothing on it. No wall, no floor, no room behind it - only the armchair against flat green. $S" $O/lounge_empty.png &
