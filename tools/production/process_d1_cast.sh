...
cd "$(dirname "$0")/../.."
PY=./.venv/bin/python
SRC=assets/sources/production/stages/dirty_delhi/cast
CUT=assets/qa/d1frames/
mkdir -p $CUT

slice() {  # slice <sheet> <prefix> <count>
  [ -s "$SRC/$1.png" ] || { echo "MISSING $1"; return; }
  $PY tools/production/slice_sheet.py "$SRC/$1.png" "$2" --expect "$3" --out "$CUT" || echo "SLICE FAILED $1"
}
...
proc() {  # proc <char> <height> <fill> <ref>
  $PY tools/production/process_char.py "$1" --height "$2" --fill "$3" --ref "$4" --colors 44 \
