...
# sprites go through tools/production/process_props.py (LAIR table); the map panel is a full-screen
# menu background and goes through tools/production/pixelate.py instead.
#
...
set -uo pipefail
cd "$(dirname "$0")/../.."
G=tools/production/gen_codex.sh
O=assets/sources/production/lair
...
P=assets/sources/production/props
F=assets/sources/production/fg
mkdir -p $O $P $F
...
# The FIGHT, not the trophy: the aftermath is already lying on the floor two feet below this
# frame, so a standing-over-the-corpse pose said the same thing twice. Of three battle
# compositions, the grapple won at final size - a mid-leap and a punch both left the cat
# floating in a corner disconnected from him, where two masses locked together in the centre
# read instantly at 76 logical px.
#
# Three things must be said as RULES or they come back wrong, all learned the hard way:
#  - the beast is THE SAME animal as the pelt below it. Described in words it came back a
#    plain tan lion, so the processed pelt sprite goes in as a second reference image.
#  - the whole scene must sit INSIDE the frame with headroom. An earlier pass ran the top of
#    his skull into the frame's inner edge and clipped it.
#  - it is sized by HEIGHT and comes out narrower than the bay, which a painting is allowed
#    to be. Forcing this near-square canvas to the bay's 87 stretched the figures 18% wide.
$G $O/overmantel.png landscape "An old master oil painting in a heavy carved gilded frame. The painting shows $CHAD locked chest to chest with the rearing sabretooth cat, gripping its open jaws in both hands and forcing them apart, their faces inches from each other, the cat reared up on its hind legs as tall as he is with its claws raking at his shoulders and his arms and back bulging with the strain. The cat is THE SAME ANIMAL as the striped pelt in the second reference image and must match it exactly: deep russet orange fur crossed by strong black tiger stripes, a heavy dark blackish-brown mane framing its face, a pale cream muzzle and chest, and two enormous curved ivory sabre fangs. Painted in the manner of a baroque battle canvas: violent diagonal composition, deep chiaroscuro, dark umber and ochre tones with cream highlights blazing on the man's back and shoulders and on the cat's fangs, storm light breaking through black cloud behind them, dust and grit thrown up around their feet. Dramatic, heroic and violent. CRITICAL COMPOSITION RULE: the ENTIRE painted scene must sit INSIDE the gilded frame with nothing touching or crossing it. There must be a clear band of sky above the highest point of the figures, about as tall as the man's head, and clear ground below the lowest point. Nothing - no head, no paw, no fang, no tail - may touch, overlap or be cut off by the frame. Zoom the scene OUT so the whole composition fits comfortably with generous margins on all four sides. The gilded frame is thick and ornate with carved acanthus scrollwork and a crest at the top centre. The whole framed painting is drawn small and centred with a wide margin of flat green all around the outside of the gilded frame. $FLAT" assets/sources/production/ref_chad.png assets/lair/bed_rug.png &
