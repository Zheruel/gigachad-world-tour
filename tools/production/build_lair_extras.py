...
def build_bed():
    """The master suite's bed, one generation per pose from assets/sources/production/lair/bed/.
    ...
    furniture, and tools/verification/check_bed_poses.py measures the result at 0.1% furniture drift.
...
def build_fender():
    ...
    """
    plate = Image.open("assets/lair/wall.png").convert("RGBA")
...
# the frame's outer edge in assets/lair/wall.png; TANK_OUT is where it goes now.
...
def build_tankframe():
    ...
    """
    src = Image.open("assets/lair/wall.png").convert("RGBA").crop(TANK_SRC)
    sw, sh = src.size
