...
RS = 2
SRC = "assets/sources/production/props/"
OUT = "assets/props/"
...
LAIR_SRC = "assets/sources/production/lair/"
...
LAIR = {
    ...
    # behind it, which is cropped off in tools/production/gen_lair_deco.sh's note.
    "lounge_chair": (70, 50),
    "overmantel": (76, 70),
...
def process(name, broken, sizes=SIZES, src_dir=SRC, out_dir=OUT, out_name=None, out_path=None, broken_h=0.45):
    ...
    if name in D1_BY_WIDTH and not broken:
        tw = int(lw * RS)
...
if __name__ == "__main__":
    names = sys.argv[1:] or list(SIZES) + list(LAIR) + list(D1)
    for n in names:
