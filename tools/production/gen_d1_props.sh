...
# Usage: tools/production/gen_d1_props.sh [props|rig|relics|inpaint|all]
# Then: ./.venv/bin/python tools/production/process_props.py <names>  (the D1 table sizes them), and
# tools/production/patch_plate.py for the inpaint.
set -uo pipefail
cd "$(dirname "$0")/../.."
G=tools/production/gen_codex.sh
O=assets/sources/production/stages/dirty_delhi/props
mkdir -p $O
WHAT="${1:-all}"
