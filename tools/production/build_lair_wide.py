...
Each selected panel appears once. Window cutouts preserve separate city parallax;
edge-local exposure matching preserves the room's warm and cool lighting zones.
...
scales uniformly instead.
  ...
  ./.venv/bin/python tools/production/build_lair_wide.py [--seams A B C] [--near-centre F]
"""
import argparse
import os
import sys
...
RS = 2
PANELS = ("assets/sources/production/lair_room5/panel_a.png",
          "assets/sources/production/lair_room5/panel_b.png",
          "assets/sources/production/lair_room5/panel_c.png",
          "assets/sources/production/lair_room5/panel_d.png")
SKY_DIR = "assets/sources/production/lair_sky/"
...
def build_room(seams, key=True):
    ...
    quantized(wall, 160).save("assets/lair/wall.png")
    quantized(floor, 96).save("assets/lair/floor.png")
...
def stitch_sky(srcs, centre, feather, key=False):
    ...
    if key:
        # Remove green BEFORE joining foreground rooftops. An RGB cross-fade
        # mixes solid roof with the adjacent panel's green sky, so the later key
        # bites into the helipad and makes its railing abruptly disappear.
        result = Image.new('RGBA', (VIEW_W * len(parts) + OVERLAP, WALL_H))
        for i, part in enumerate(parts):
            result.alpha_composite(key_glass(part), (i * VIEW_W, 0))
        inset = OVERLAP // 2
        return result.crop((inset, 0, inset + VIEW_W * len(parts), WALL_H))
...
def build_sky(far_centre=0.5, near_centre=0.46):
    ...
    for name, srcs, centre, colors, feather, key in (
        ...
        if not paths:
            continue
        layer = stitch_sky(paths, centre, feather, key=key)
        quantized(layer, colors).save(f"assets/lair/sky_{name}.png")
