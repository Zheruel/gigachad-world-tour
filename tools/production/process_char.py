...
SRC_DIR = "assets/sources/production/frames/"
...
def clean_walk_fragments(img):
    """CHAD's walk crops include disconnected pieces of neighboring boots."""
    img = img.convert('RGBA')
    alpha = img.getchannel('A')
    remaining = {(x, y) for y in range(img.height) for x in range(img.width)
    ...
    if len(components) > 1:
        ...
        for point in body:
            mask.putpixel(point, alpha.getpixel(point))
        img.putalpha(mask)
    return img
...
def main():
    ...
    for path in paths:
        ...
        image = hard_alpha(key_green(Image.open(path)))
        if out_prefix == 'chad' and name.startswith('swlk'):
            image = clean_walk_fragments(image)
        keyed[name] = subject(image)
