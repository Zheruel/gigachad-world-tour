#!/bin/bash
# Rebuild every shipped CHAD family without an artificial silhouette outline.
set -euo pipefail
cd "$(dirname "$0")/../.."
PY=./.venv/bin/python

$PY tools/production/process_char.py chad --height 192 --fill .94 --ref sidle1 --colors 48 \
  --src-prefix chad --out-prefix chad --src-dir assets/sources/production/chad/ --no-outline

$PY tools/production/process_char.py parry_counter --height 192 --fill .94 --ref 1 --colors 48 \
  --src-prefix parry_counter --out-prefix chad_parry_counter \
  --src-dir assets/sources/production/chad_v2/ --no-outline

$PY tools/production/process_char.py combo_power_a --height 192 --fill .94 --ref 1 --colors 48 \
  --src-prefix combo_power_a --out-prefix chad_combo_power_a \
  --src-dir assets/sources/production/chad_v4/ --no-outline
# Frame 1 is crouched. Using it as the scale reference enlarged the entire
# second half of the combo; frame 8 is the neutral standing-height calibration.
$PY tools/production/process_char.py combo_power_b --height 192 --fill .94 --ref 8 --colors 48 \
  --src-prefix combo_power_b --out-prefix chad_combo_power_b \
  --src-dir assets/sources/production/chad_v4/ --no-outline
$PY tools/production/process_char.py ragnarok_ground --height 192 --fill .94 --ref 1 --colors 48 \
  --src-prefix ragnarok_ground --out-prefix chad_ragnarok_ground \
  --src-dir assets/sources/production/chad_v4/ --no-outline
$PY tools/production/process_char.py ragnarok_air --height 256 --fill .70 --ref 1 --colors 48 \
  --src-prefix ragnarok_air --out-prefix chad_ragnarok_air \
  --src-dir assets/sources/production/chad_v4/ --no-outline
# RAGNAROK has no standing pose at all - every frame leans into the spin - so calibrating
# any of them to a standing figure's .94 inflated the whole family and CHAD visibly grew
# when he supered. Measured on the one landmark that is the same size in every pose, the
# crown of his skull: 27.5 px across here against 23.0 in the idle frames, so .94 x 23/27.5.
# Same mistake as combo_power_b above, and the same tell.
$PY tools/production/process_char.py super_express --height 192 --fill .786 --ref 8 --colors 48 \
  --src-prefix super_express --out-prefix chad_meteor_lariat \
  --src-dir assets/sources/production/chad_v3/ --no-outline

$PY tools/production/build_manifest.py --prune
$PY tools/production/process_ambience.py
