...
> are historical; runtime code is authoritative.
...
**MIRCHI** is generated by `tools/production/gen_d1_mirchi.sh`: one reference, seven strips (idle, walk,
...
bar; `tools/production/process_d1_cast.sh` scales it up before processing.
...
The production arrival uses three registered riding cels and the parked motorcycle.
CHAD dismounts under procedural dust and continues with his normal in-game poses.
The timeline is `drawMotorcycleArrival()` in `js/story.js`.
Selected source inputs and prompts live in `assets/sources/motorcycle/`;
`tools/production/build_motorcycle.py` and `build_entrance_bike.py` rebuild them.
